...
function EditOrderPage() {
  ...
  const existingItems = itemsQuery.data ?? [];
  ...
  useEffect(() => {
    ...
    for (const it of existingItems) {
      const product =
        ...
        products.find((p) => p.code === it.product_code);
      if (!product) continue;
      nextQty[product.id] = it.quantity;
      if ((it.price ?? "").trim()) nextPrice[product.id] = it.price ?? "";
    }

    setQuantities(nextQty);
    setPrices(nextPrice);
    setHydrated(true);
  }, [hydrated, loading, order, existingItems, products]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      try {
        ...
        const blob = await generatePdfBlob(pdfDataFromOrder(updatedOrder, itemRows));
        await uploadOrderPdf(user.id, order.id, blob);
      } catch (pdfErr) {
        console.warn("[pdf] upload after edit failed", pdfErr);
      }

      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.myOrders }),
        queryClient.invalidateQueries({ queryKey: queryKeys.order(id) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.orderItems(id) }),
      ]);

      toast.success(`Order Requisition ${order.document_number} updated`);
      navigate({ to: "/orders/$id", params: { id: order.id } });
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "Failed to update order requisition";
      toast.error(msg);
    } finally {
      setSubmitting(false);
    }
